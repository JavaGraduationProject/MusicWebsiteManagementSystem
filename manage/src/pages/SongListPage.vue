  ...
  <div class="table">
    <div class="container">
      <div class="handle-box">
        ...
        <el-input v-model="select_word" size="small" placeholder="请输入歌单名" class="handle-input mr10"></el-input>
        <el-button type="primary" size="small" @click="centerDialogVisible = true"
          style="background-color: #8bc24c; border: 0px; font-size: 15px;">添加歌单</el-button>
      </div>
      <el-table :data="data" border size="mini" style="width: 100%" height="550px" ref="multipleTable"
        ...
        <el-table-column prop="title" label="歌单名" width="200" align="center"></el-table-column>
        <el-table-column label="简介">
          <template slot-scope="scope">
            <p style="height: 100px; overflow: scroll">{{ scope.row.introduction }}</p>
          </template>
        </el-table-column>
        <el-table-column label="评分" prop="ranks" width="150" align="center" sortable
          :default-sort="{ prop: 'ranks', order: 'ascending' }"></el-table-column>
        <el-table-column label="风格" width="150" align="center" prop="style" :filters="getfilterNameItem()"
          :filter-method="filterHandler" :filter-multiple="false">
          <template slot-scope="scope">
            <div>{{ scope.row.style }}</div>
          </template>
        </el-table-column>
        <el-table-column label="歌曲管理" width="150" align="center">
          <template slot-scope="scope">
            <el-button size="mini" @click="getContent(data[scope.$index].id)">歌曲管理</el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog title="添加歌单" :visible.sync="centerDialogVisible" width="400px" center>
      <el-form :model="registerForm" status-icon ref="registerForm" label-width="70px" class="demo-ruleForm">
        ...
        <el-form-item label="歌单介绍" prop="introduction" size="mini">
          <el-input type="textarea" v-model="registerForm.introduction" placeholder="歌单介绍"></el-input>
        </el-form-item>
...
import { setSongList, getSongList, updateSongList, deleteSongList,getRanksOfSongListId } from '../api/index'
  ...
  methods: {
    ...
    getData() {
      ...
      getSongList().then((res) => {
        for (let item of res) {
          this.getSongListRanks(item.id, item)
        }
        this.currentPage = 1
      })
    },
    ...
    getfilterNameItem() {
      ...
      for (let i in this.styles) {
        for (let j in this.data) {
          if (this.data[j].style == this.styles[i].value) {
            apiArr.push({
              text: this.styles[i].value,
    ...
    addsongList() {
      ...
      params.append('pic', '/img/songListPic/list.jpeg')
