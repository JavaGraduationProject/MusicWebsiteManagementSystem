<template>
  <div class="the-header">
    ...
    <ul class="navbar" ref="change">
      ...
      <li v-if="!loginIn" :class="{ active: item.name === activeName }" v-for="item in loginMsg" :key="item.type"
        @click="goPage(item.path, item.name)">{{ item.name }}</li>
    </ul>
    <!--设置-->
...
import {loginMsg, menuList, navMsg} from '../assets/data/header'
...
export default {
  ...
  methods: {
    ...
    goPage(path, value) {
      ...
      if (!this.loginIn && (path != '/' && path != '/sign-up')) {
    ...
    goMenuList(path) {
      if (path == 0) {
        ...
        // this.$store.commit('setSearchword', false)
        // this.$store.commit('setIsPaly', false)
