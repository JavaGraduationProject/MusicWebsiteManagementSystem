const song = {
  ...
  getters: {
    ...
    price: state => {
      ...
      if (!price) {
        price = JSON.parse(window.sessionStorage.getItem('price') || 0.0)
      }
      return price
    },
    clickTimes: state => {
      ...
      if (!clickTimes) {
        clickTimes = JSON.parse(window.sessionStorage.getItem('clickTimes') || 0)
      }
      return clickTimes
    },
    url: state => {
    ...
    picUrl: state => {
      ...
      if (!picUrl) {
        picUrl = JSON.parse(window.sessionStorage.getItem('picUrl')) || 'http://localhost:8888/img/songPic/default.jpeg'
      }
      return picUrl
    },
    autoNext: state => {
  ...
  mutations: {
    ...
    setclickTimes: (state, clickTimes) => {
      state.clickTimes = clickTimes
      window.sessionStorage.setItem('clickTimes', JSON.stringify(clickTimes))
    },
    setUrl: (state, url) => {
      state.url = url
